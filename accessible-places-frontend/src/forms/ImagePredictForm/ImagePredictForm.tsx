...
export function ImagePredictForm() {
  ...
  const {
    formData: { imageRaw, imageBase64, coordinates, poiName, boundingBoxes },
    saveData,
  ...
  const { predictions, savePredictions } = useContext(
    PredictionContext
  ...
  const onSubmit = async () => {
    ...
    for (const [index, img] of imageArr.entries()) {
      ...
      photos.push({
        ...
        detections: covertBoxToPrediction(
          boundingBoxes[index],
          predictions[index]
        ),
      });
    ...
    if (history.state.usr) {
      ...
      await updatePlace({
        ...
        isAccessible:
          historyState.isAccessible || checkAccessibility(predictions),
        photos: [
          ...historyState.photos.map((photo) => {
            return {
              id: photo.id,
              url: photo.url,
    ...
    <Flex direction={"column"} justifyContent="space-between">
      ...
      {!loading && (
        <Flex gap={7} direction="column">
          {imageRaw &&
            Array.from(imageRaw).map((_image, id) => (
              <ImagePredictionCard
                predictionID={id}
                selectedFile={imageBase64[id]}
                predictions={predictions[id]}
                key={`prediction_card_${id}`}
              />
            ))}
        </Flex>
      )}
      <Flex gap="2" py={8} w="100%" justifyContent="space-between">
        <Button
